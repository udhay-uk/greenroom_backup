...
import { 
  ChevronLeft, Save, Info, AlertCircle, FileCheck,
  Upload, X, Check, PenTool, Image as ImageIcon
} from 'lucide-react';
import {
  Box,
  Button,
...
const SignatureSetup: React.FC = () => {
  ...
  const [uploadedSignature, setUploadedSignature] = useState<SignatureData | null>(null);
    ...
    if (validateForm()) {
      ...
      setTimeout(() => {
        ...
        console.log('Form submitted with data:', {
          signaturePolicy,
          signatureMethod,
          signature: signatureMethod === 'draw' ? drawnSignature : uploadedSignature
        });
      }, 1500);
    }
  };
  ...
  return (
    <Box sx={{ backgroundColor: 'grey.50', minHeight: '100vh' }}>
      <Box component="header" sx={{ backgroundColor: 'background.paper', boxShadow: 1 }}>
        <Box sx={{ 
          maxWidth: '7xl',
          mx: 'auto',
          px: { xs: 2, sm: 3, lg: 4 },
        ...
        <Card>
          ...
          <CardContent>
            <Alert 
              ...
              sx={{ mb: 3 }}
            >
              Your signature will be used on checks and official payroll documents. You can choose to have one signature or require two signatures for added security.
            </Alert>
            
            <Box component="form" onSubmit={handleSubmit} sx={{ '& > * + *': { mt: 3 } }}>
              {/* Signature Policy */}
              <FormControl fullWidth error={!!errors.signaturePolicy}>
                <InputLabel id="signaturePolicy-label">Signature Policy *</InputLabel>
                <Select
                ...
                <FormHelperText>
                  Single Signature: One signature required on checks and documents.
                  <br />
                  Double Signature: Two signatures required for enhanced security.
                </FormHelperText>
              </FormControl>
                ...
                {signatureMethod === 'draw' && (
                  <Paper variant="outlined" sx={{ p: 2 }}>
                    ...
                    {errors.signatureDraw && (
                      <Alert severity="error" icon={<AlertCircle size={16} />} sx={{ mt: 1 }}>
                        {errors.signatureDraw}
                      </Alert>
                    )}
                  </Paper>
                      ...
                      <>
                        <label htmlFor="signatureUpload">
                          <UploadArea elevation={0}>
                            <Upload size={40} color="#9ca3af" style={{ marginBottom: '8px' }} />
              ...
              {/* Form Actions */}
              <Stack direction="row" justifyContent="flex-end" spacing={2} pt={3} sx={{ borderTop: 1, borderColor: 'divider' }}>
                <Button
                  variant="outlined"
                  sx={{ textTransform: 'none' }}
                >
