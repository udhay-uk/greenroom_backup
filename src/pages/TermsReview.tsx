...
const TermsReview: React.FC = () => {
  ...
  const agreements: Agreement[] = [
    { id: 'terms', title: 'Terms of Service', fileName: 'greenroom-terms-of-service.pdf' },
    { id: 'privacy', title: 'Privacy Policy', fileName: 'greenroom-privacy-policy.pdf' },
    { id: 'dataProcessing', title: 'Data Processing Agreement', fileName: 'greenroom-dpa.pdf' },
    { id: 'security', title: 'Security Policy', fileName: 'greenroom-security-policy.pdf' },
  ];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    setTimeout(() => {
      console.log('Submitted', { ...registrationData, agreements: acceptedAgreements });
      setSubmitting(false);
    }, 1500);
  };
  ...
  const handleDownload = (fileName: string) => console.log(`Downloading ${fileName}`);

  const formatDate = (dateStr: string) => new Date(dateStr).toLocaleDateString(undefined, {
    year: 'numeric', month: 'long', day: 'numeric'
  });

  const allAgreementsAccepted = Object.values(acceptedAgreements).every(Boolean);

  return (
    <Box maxWidth="md" mx="auto" p={3} component={Paper} elevation={3}>
      <Typography variant="h5" fontWeight="bold" gutterBottom>Registration</Typography>
      <Typography color="text.secondary">Step 4 of 4: Review & Terms</Typography>
      ...
      <Box mt={4}>
        ...
        <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
          <Typography variant="subtitle1" fontWeight="medium" gutterBottom>Company Information</Typography>
          <Divider sx={{ mb: 2 }} />
        ...
        <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
          <Typography variant="subtitle1" fontWeight="medium" gutterBottom>Administrator</Typography>
          <Divider sx={{ mb: 2 }} />
          <Stack spacing={1} direction="row" flexWrap="wrap" gap={4}>
            <Box width={"45%"}>
        ...
        <Paper variant="outlined" sx={{ p: 2 }}>
          <Typography variant="subtitle1" fontWeight="medium" gutterBottom>Payroll Settings</Typography>
          <Divider sx={{ mb: 2 }} />
          <Stack spacing={1} direction="row" flexWrap="wrap" gap={4}>
            <Box width={"45%"}>
            ...
            <Box width={"45%"}>
              ...
              <Typography>{formatDate(registrationData.payroll.payrollStartDate)}</Typography>
            </Box>
            <Box width={"45%"}>
              <Typography variant="caption" color="text.secondary">Starting Check Number</Typography>
              <Typography>{registrationData.payroll.startingCheckNumber}</Typography>
            </Box>
          </Stack>
        </Paper>
      </Box>
      ...
      <Box component="form" onSubmit={handleSubmit} mt={4}>
        ...
        <Stack spacing={2}>
          {agreements.map((agreement) => (
            <Box key={agreement.id} display="flex" alignItems="center" justifyContent="space-between" p={2} border="1px solid" borderColor="grey.300" borderRadius={1}>
              <FormControlLabel
                control={
                  <Checkbox
              ...
              />
              <Button onClick={() => handleDownload(agreement.fileName)} startIcon={<Download />}>
                Download
              </Button>
            </Box>
          ))}
        ...
        <Stack direction="row" justifyContent="space-between" mt={4}>
          <Button variant="outlined" startIcon={<ChevronLeft />} onClick={handleBack}>
            Back
          </Button>
          <Button
            type="submit"
